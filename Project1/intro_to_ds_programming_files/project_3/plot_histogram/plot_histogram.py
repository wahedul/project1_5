...
def entries_histogram(turnstile_weather):
    ...
    '''
    print turnstile_weather['DATEn']
    turnstile_weather['weekday']= turnstile_weather['DATEn'].map(lambda x: x.strftime('%A'))
    #turnstile_weather['weekday']
    print turnstile_weather['weekday']
    '''
    p=[]
    for i in range (24):
    ...
    plt.show()
    '''
    turnstile_weather['ENTRIESn_hourly'][turnstile_weather.rain==1].hist(bins=20) # your code here to plot a historgram for hourly entries when it is raining
    turnstile_weather['ENTRIESn_hourly'][turnstile_weather.rain==0].hist(bins=20) # your code here to plot a historgram for hourly entries when it is not raining
    plt.show()
    return plt
